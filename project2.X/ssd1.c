...
#define TMR_TIME    0.003125 // 3.125 ms for each tick 
...
unsigned int SSD_Flag = 0;
unsigned int SSD_Writeflag = 0;
unsigned int LCD_Writeflag = 0;
...
{  
    ...
    if(baseCnt++ == 9)     
    {
        ...
        if(SWT_GetValue(5)) switch_flag5=1;
        else switch_flag5=0;
        if(SWT_GetValue(6)) switch_flag6=1;
        else switch_flag6=0;
        
        if(SWT_GetValue(7)) switch_flag7=1;
        else switch_flag7=0;
...
{
  PR1 = (int)(((float)(TMR_TIME * PB_FRQ) / 64)+17); //set period register, generates one interrupt every 3.125 ms
